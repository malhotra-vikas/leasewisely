...
import * as CustomLogger from "../utils/GolfProAI-logger"
...
export async function createUser(userEmail: string, name: string) {
  ...
  try {
    ...
    console.log("Created in table: " + Constants.GOLF_PRO_USERS_TABLE)
...
export async function updateContact(email: string, name: string, handicap: string, currentBallBrand: string, 
  currentBallModel: string, golfState: string, driverDistance: string, driverTrajectory: string, 
  driverShotShape: string, driverSpinRate: string, iron7Distance: string, iron7Trajectory: string, 
  abilityToShapeShots: string, greenSideSpin: string, wedgesSpinControl: string, puttingFeel: string, 
  criticalCharacteristics: string, idealBallDescription: string, discoveryQuestionCount: any) {
  try {
    ...
    if (handicap && handicap.trim() !== '') {
      updateExpressionParts.push('#handicap = :handicap')
      expressionAttributeNames['#handicap'] = 'handicap'
      expressionAttributeValues[':handicap'] = handicap
    }

    if (currentBallBrand && currentBallBrand.trim() !== '') {
      updateExpressionParts.push('#currentBallBrand = :currentBallBrand')
      expressionAttributeNames['#currentBallBrand'] = 'currentBallBrand'
      expressionAttributeValues[':currentBallBrand'] = currentBallBrand
    }

    if (currentBallModel && currentBallModel.trim() !== '') {
      updateExpressionParts.push('#currentBallModel = :currentBallModel')
      expressionAttributeNames['#currentBallModel'] = 'currentBallModel'
      expressionAttributeValues[':currentBallModel'] = currentBallModel
    }

    if (golfState && golfState.trim() !== '') {
      updateExpressionParts.push('#golfState = :golfState')
      expressionAttributeNames['#golfState'] = 'golfState'
      expressionAttributeValues[':golfState'] = golfState
    }

    if (driverDistance && driverDistance.trim() !== '') {
      updateExpressionParts.push('#driverDistance = :driverDistance')
      expressionAttributeNames['#driverDistance'] = 'driverDistance'
      expressionAttributeValues[':driverDistance'] = driverDistance
    }

    if (driverTrajectory && driverTrajectory.trim() !== '') {
      updateExpressionParts.push('#driverTrajectory = :driverTrajectory')
      expressionAttributeNames['#driverTrajectory'] = 'driverTrajectory'
      expressionAttributeValues[':driverTrajectory'] = driverTrajectory
    }

    if (driverShotShape && driverShotShape.trim() !== '') {
      updateExpressionParts.push('#driverShotShape = :driverShotShape')
      expressionAttributeNames['#driverShotShape'] = 'driverShotShape'
      expressionAttributeValues[':driverShotShape'] = driverShotShape
    }    

    if (driverSpinRate && driverSpinRate.trim() !== '') {
      updateExpressionParts.push('#driverSpinRate = :driverSpinRate')
      expressionAttributeNames['#driverSpinRate'] = 'driverSpinRate'
      expressionAttributeValues[':driverSpinRate'] = driverSpinRate
    }    
    if (iron7Distance && iron7Distance.trim() !== '') {
      updateExpressionParts.push('#iron7Distance = :iron7Distance')
      expressionAttributeNames['#iron7Distance'] = 'iron7Distance'
      expressionAttributeValues[':iron7Distance'] = iron7Distance
    }    
    if (iron7Trajectory && iron7Trajectory.trim() !== '') {
      updateExpressionParts.push('#iron7Trajectory = :iron7Trajectory')
      expressionAttributeNames['#iron7Trajectory'] = 'iron7Trajectory'
      expressionAttributeValues[':iron7Trajectory'] = iron7Trajectory
    }    
    if (abilityToShapeShots && abilityToShapeShots.trim() !== '') {
      updateExpressionParts.push('#abilityToShapeShots = :abilityToShapeShots')
      expressionAttributeNames['#abilityToShapeShots'] = 'abilityToShapeShots'
      expressionAttributeValues[':abilityToShapeShots'] = abilityToShapeShots
    }    

    if (greenSideSpin && greenSideSpin.trim() !== '') {
      updateExpressionParts.push('#greenSideSpin = :greenSideSpin')
      expressionAttributeNames['#greenSideSpin'] = 'greenSideSpin'
      expressionAttributeValues[':greenSideSpin'] = greenSideSpin
    }
    if (wedgesSpinControl && wedgesSpinControl.trim() !== '') {
      updateExpressionParts.push('#wedgesSpinControl = :wedgesSpinControl')
      expressionAttributeNames['#wedgesSpinControl'] = 'wedgesSpinControl'
      expressionAttributeValues[':wedgesSpinControl'] = wedgesSpinControl
    }
    if (puttingFeel && puttingFeel.trim() !== '') {
      updateExpressionParts.push('#puttingFeel = :puttingFeel')
      expressionAttributeNames['#puttingFeel'] = 'puttingFeel'
      expressionAttributeValues[':puttingFeel'] = puttingFeel
    }
    if (criticalCharacteristics && criticalCharacteristics.trim() !== '') {
      updateExpressionParts.push('#criticalCharacteristics = :criticalCharacteristics')
      expressionAttributeNames['#criticalCharacteristics'] = 'criticalCharacteristics'
      expressionAttributeValues[':criticalCharacteristics'] = criticalCharacteristics
    }
    if (idealBallDescription && idealBallDescription.trim() !== '') {
      updateExpressionParts.push('#idealBallDescription = :idealBallDescription')
      expressionAttributeNames['#idealBallDescription'] = 'idealBallDescription'
      expressionAttributeValues[':idealBallDescription'] = idealBallDescription
    }

    if (discoveryQuestionCount) {
      updateExpressionParts.push('#discoveryQuestionCount = :discoveryQuestionCount')
      expressionAttributeNames['#discoveryQuestionCount'] = 'discoveryQuestionCount'
      expressionAttributeValues[':discoveryQuestionCount'] = discoveryQuestionCount
    }
    ...
    console.log("Params added as" + updateExpression)
    console.log("Params added as" + expressionAttributeValues)
